...
public class BenchmarkingClientGuiFx2 extends Application
	...
	// BenchmarkingClient Initialisieren
	...
	private Pane createRunTimePane() {
		...
		runTimePane.add(createLabel("Max. geplante Event-Nachrichten"), 1, 5);
		...
		// Luecken zwischen den Objekten festlegen und Ausrichtung definieren
	...
	private Pane createResultPane() {
		...
		// Luecken zwischen den Objekten festlegen
	...
	private ProgressBar createProgressbar() {
		...
		progressBar.setMinSize(1210, 30);
		progressBar.setMaxSize(1210, 30);
	...
	private HBox createSeperator(String value) {
		...
		final double labelLength = value.length();
	...
	/**
	 * Es wird ein Task erstellt, der den Ablauf des Programms simuliert und
	 * immer, wenn der progressCounter erhoeht wird seinen Fortschritt erhoeht
	 * 
	 * @return Task
	 */
	 ...
	 */
	private synchronized void countUpProgressBar() {
	...
	private void setNumberOfClientThreads() {
		...
		if (testString3.matches("[0-9]+")) {
			...
			if (iClientThreads < 1 || iClientThreads > 5000) {
				// Nicht im Wertebereich
				// Aktualisieren des Frames auf dem Bildschirm
				startable = false;
	...
	/**
	 * Progressbar weiterstellen Methode, die immer aufgerufen wird, wenn eine
	 * Response-Nachricht empfangen, oder ein Client ein- oder ausgeloggt wird
	 */
